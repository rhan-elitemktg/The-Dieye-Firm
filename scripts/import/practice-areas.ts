...
const CONTENT = join(process.cwd(), "src/content/practice-areas");

/** The section root renders at /family-law/, not /family-law/family-law/. */
const SECTION_ROOT_ID = "family-law";
...
const idFor = (contentId: string) =>
  `practiceArea-${contentId.replace(/\//g, "-")}`;

async function run() {
  const files = walk(CONTENT).sort();
  if (files.length !== 32) {
    throw new Error(`Expected 32 practice areas, found ${files.length}`);
  }
  ...
  /* Every parent must resolve, or the sidebar silently loses a branch. */
  for (const d of docs) {
    if (d.parent && !seenIds.has(d.parent._ref)) {
      throw new Error(`${d._id} has parent "${d.parent._ref}" which is not among the imported documents`);
    }
  }
  if (!seenSlugs.has(SECTION_ROOT_ID)) {
    throw new Error(`No "${SECTION_ROOT_ID}" document — the section root renders at /family-law/`);
  }
  ...
  const roots = docs.filter((d) => !d.parent).length;
  console.log(`✓ ${docs.length} practice areas written  (${roots} top-level, ${docs.length - roots} nested)`);

  await waitForPublic(
    'count(*[_type == "practiceArea"])',
    docs.length,
    `${docs.length} practice areas`,
  );
}
